...
const estimateSchema = z.object({
    ...
    taxRate: z.number(),
    conditions: z.string(),
    details: z.array(estimateDetailSchema),
    companies: z.array(estimateCompanySchema),
});
...
export default function EstimateForm() {
    ...
    useEffect(() => {
        ...
        if (savedData) {
            console.log(`getFormItem ${(JSON.parse(savedData) as FormValues).reference}`)
            var parsedData = JSON.parse(savedData) as FormValues;
            setBase64String(parsedData?.logo);
            setValue('reference', parsedData?.reference);
            setValue('subjectMatter', parsedData?.subjectMatter);
            setValue('details', parsedData?.details);
            setValue('companies', parsedData?.companies);
            setValue('taxRate', parsedData?.taxRate);
            setValue('conditions', parsedData?.conditions);
        }
        setIsLoading(false);
    }, []);
    ...
    var form = useForm<FormValues>({
        ...
        defaultValues: {
            ...
            logo: base64String,
            subjectMatter: t('subjectMatterText'),
            creationDate: new Date(),
            updateDate: new Date(),
            expiracyDate: new Date(date.setMonth(date.getMonth() + 3)),
            currency: "EUR",
            taxRate: 20,
            conditions: t('conditionsText'),
            details: [
                {
                    id: 1,
                    estimateId: 1,
                    rawDescription: t('descriptionText1'),
                    quantity: 50,
                    unitPrice: 100,
                ...
                },
                {
                    id: 2,
                    estimateId: 1,
                    rawDescription: t('descriptionText2'),
                    quantity: 60,
                    unitPrice: 120,
                    creationDate: new Date('2024-01-01'),
                    updateDate: new Date('2024-01-15'),
    ...
    useEffect(() => {
        if (!isLoading) {
            console.log(`setFormItem ${watchFields.reference}`);
    ...
    function addDetail() {
        const detailLength = form.watch('details').length;
    ...
    async function onSubmit(values: FormValues) {
        const estimate: Estimate = {
            ...
            companies: values.companies.map((company, index) => ({
                id: company.id,
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input placeholder={t('reference')} {...field} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            ...
                            <Tabs defaultValue="issuer">
                                ...
                                {form.watch('companies').map((company, index) => (
                                    <TabsContent className="max-w-md space-y-4 p-1"
                                        ...
                                        <FormField
                                            control={form.control}
                                            name={`companies.${index}.businessName`}
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>
                                                        {t('businessName')}
                                        ...
                                        <FormField
                                            control={form.control}
                                            name={`companies.${index}.businessAddress`}
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>
                                                        {t('businessAddress')}
                                        ...
                                        <FormField
                                            control={form.control}
                                            name={`companies.${index}.taxNumber`}
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>
                                                        {t('taxNumber')}
                                        ...
                                        <FormField
                                            control={form.control}
                                            name={`companies.${index}.siretNumber`}
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>
                                                        {t('siretNumber')}
                                        ...
                                        <FormField
                                            control={form.control}
                                            name={`companies.${index}.sirenNumber`}
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>
                                                        {t('sirenNumber')}
                            ...
                            <Tabs value={tab} onValueChange={onTabChange}>
                                ...
                                {form.watch('details').map((detail, index) => (
                                    <TabsContent className="max-w-md space-y-4 p-1"
                                        ...
                                        <FormField
                                            control={form.control}
                                            name={`details.${index}.rawDescription`}
                                            render={({ field }) => (
                                                <FormItem>
                                                    <FormLabel>
                                                        {t('description')}
